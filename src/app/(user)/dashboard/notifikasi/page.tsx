...
export default function NotifikasiPage() {
  ...
  // 👇 FITUR UTAMA: Aksi Klik Notifikasi (Read & Cerdas Redirect)
  const handleNotifClick = async (notif: any) => {
    try {
      // 1. Tandai sebagai telah dibaca di backend (background process)
      if (!notif.isRead) {
      ...
      // 2. Tentukan halaman tujuan berdasarkan Type/Kategori Notifikasi
      const type = notif.type?.toUpperCase() || '';
      let targetUrl = '';

      // Kasus A: Jika sudah ada URL spesifik di metadata (dari backend)
      if (notif.metadata?.url) {
         targetUrl = notif.metadata.url;
      } 
      // Kasus B: Redirect otomatis berdasarkan Type Notifikasi
      else if (type.includes('INVOICE') || type.includes('PAYMENT') || type.includes('BILL')) {
         targetUrl = '/dashboard/tagihan';
      } 
      else if (type.includes('ACTIVATE') || type.includes('SUSPEND') || type.includes('CANCEL') || type.includes('PACKAGE') || type.includes('ADDRESS')) {
         targetUrl = '/dashboard/layanan';
      } 
      else if (type.includes('WELCOME')) {
         targetUrl = '/dashboard';
      }

      // 3. Eksekusi Redirect jika targetUrl ditemukan
      if (targetUrl) {
        router.push(targetUrl);
      } else {
        // Fallback: Jika tidak ada target URL, kasih tahu aja
  ...
  const handleDeleteAll = async () => {
    if (!confirm('Apakah Anda yakin ingin menghapus SEMUA notifikasi? Aksi ini tidak dapat dibatalkan.')) return;
    
    try {
      // Nembak endpoint delete-all di backend
      await api.delete('/notifications/delete-all');
      // Kosongkan daftar di frontend secara instan
      setNotifications([]);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="text-xs font-bold text-red-500 hover:text-white bg-red-500/10 hover:bg-red-500 px-4 py-2 rounded-xl transition-all flex items-center gap-2 border border-red-500/20"
            >
            ...
            notifications.map((notif) => (
              ...
              >
                ...
                <button 
                  ...
                  className="absolute right-4 bottom-4 p-2 text-white/20 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all opacity-0 group-hover:opacity-100 z-10"
